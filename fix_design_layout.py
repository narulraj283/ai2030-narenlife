...
class HTMLFixer:
    ...
    def wrap_content_in_container(self, page_type: str = "generic") -> bool:
        ...
        if "page-container" in self.content or "article-page-container" in self.content:
            return True

        # Find the closing tag of site-header
        header_close = self.content.find('</header>')
        if header_close == -1:
            return False

        # Find the opening tag of site-footer
        footer_open = self.content.find('<footer')
        if footer_open == -1:
            return False

        # Move past the end of header tag
        content_start = header_close + len('</header>')

        # Check for mobile-overlay div and skip it
        if '<div class="mobile-overlay' in self.content[content_start:footer_open]:
            overlay_end = self.content.find('</div>', content_start)
            # Make sure this is the mobile-overlay
            overlay_start = self.content.find('<div class="mobile-overlay', content_start)
            if overlay_start > -1 and overlay_end > -1:
                # Count divs to find the correct closing tag
                div_count = 1
                temp_pos = overlay_start + len('<div class="mobile-overlay')
                while div_count > 0 and temp_pos < len(self.content):
                    next_open = self.content.find('<div', temp_pos)
                    next_close = self.content.find('</div>', temp_pos)
                    if next_close == -1:
                        break
                    if next_open > -1 and next_open < next_close:
                        div_count += 1
                        temp_pos = next_open + 1
                    else:
                        div_count -= 1
                        temp_pos = next_close + len('</div>')

                if div_count == 0:
                    overlay_end = temp_pos
                    content_start = overlay_end

        # Insert opening container div
        self.content = (
            self.content[:content_start] + '\n<div class="page-container">\n' +
            self.content[content_start:]
        )

        # Update footer_open position due to insertion
        footer_open = self.content.find('<footer')

        # Insert closing container div before footer
        self.content = (
            self.content[:footer_open] + '</div>\n' +
            self.content[footer_open:]
        )
...
def main():
    ...
    print("=" * 80)
    print("LEAD THE SHIFT - DESIGN AND LAYOUT FIX")
    print("=" * 80)
    print()
