#!/usr/bin/env python3
"""
Comprehensive QA Audit for AI2030 Repository
...
def check_navigation_consistency():
    ...
    for html_file in html_files:
        ...
        if nav_match:
            ...
            unexpected = [item for item in nav_items if item not in EXPECTED_NAV]
            if unexpected:
                nav_issues.append({
                    "file": str(html_file.relative_to(REPO_PATH)),
...
def check_company_format():
    ...
    if articles_dir.exists():
        ...
        # Check company browse pages for links to employee/investor articles
        browse_companies = REPO_PATH / "browse" / "companies"
        if browse_companies.exists():
            for browse_file in browse_companies.glob("*.html"):
                with open(browse_file, 'r', encoding='utf-8') as f:
                    content = f.read()

                if "-employee" in content or "-investor" in content:
                    format_issues.append({
                        "file": str(browse_file.relative_to(REPO_PATH)),
                        "issue": "Links to non-CEO company articles found",
                        "severity": "CRITICAL"
                    })
...
def generate_markdown_report(report):
    ...
    if report["critical"]:
        ...
        for i, issue in enumerate(report["critical"], 1):
            ...
            if "broken_link" in issue:
                md.append(f"**Broken Link:** `{issue['broken_link']}`")
            if "issue" in issue:
    ...
    if report["warnings"]:
        ...
        for issue in report["warnings"]:
            ...
            warning_groups[file].append(issue)

        for file, issues in sorted(warning_groups.items())[:20]:  # Limit to 20
            md.append(f"\n**{file}**")
    ...
    # Info
    if report["info"]:
        md.append(f"\n## Info Messages ({len(report['info'])} items)")
        md.append("See JSON report for details.")
...
if __name__ == "__main__":
    ...
    if audit_report['critical']:
        ...
        for issue in audit_report['critical'][:5]:
