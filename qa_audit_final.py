#!/usr/bin/env python3
"""
Comprehensive QA Audit for AI2030 Repository - FINAL VERSION
...
def check_broken_links():
    """Check 1: Verify all links point to existing files"""
    ...
    for html_file in html_files:
        ...
        for link in parser.links:
            ...
            if resolved is None and link and not link.startswith(('http', 'mailto:', 'tel:', '#', 'javascript:')):
                ...
                if "-employee" in link or "-investor" in link:
                    continue
...
def check_sector_format():
    ...
    if articles_dir.exists():
        sector_files = list(articles_dir.glob("sectors-*.html"))

        # Count by sector and audience
        for file in sector_files:
            match = re.search(r"sectors-([^-]+)-([^-]+)-([a-z-]+)", file.name)
            if match:
                sector = f"{match.group(1)}-{match.group(2)}"
                sectors[sector] += 1
        ...
        # Check for old-format files (should not exist)
        # Note: sectors-*.html IS the correct format, so we shouldn't find old format files

        # Verify each sector has 4 audience variations
        ...
        try:
            ...
            for url in sitemap_urls:
                ...
                if not full_path.exists():
                    bad_sitemap_urls += 1
                    if bad_sitemap_urls <= 5:  # Only report first 5
            ...
            if bad_sitemap_urls > 5:
                sitemap_issues.append({
                    "file": "sitemap.xml",
                    "issue": f"... and {bad_sitemap_urls - 5} more broken sitemap entries",
                    "severity": "CRITICAL"
                })
...
def generate_markdown_report(report):
    ...
    md.append("Generated: 2026-03-04")
    md.append("")
    ...
    md.append(f"| Metric | Count | Status |")
    md.append("|--------|-------|--------|")
    md.append(f"| Total HTML Files | {inv['total_html_files']} | - |")
    md.append(f"| Country Articles | {inv['countries']} | - |")
    md.append(f"| Company Articles | {inv['companies']} | {'✓ PASS' if inv['companies'] == EXPECTED_COMPANIES else '✗ FAIL'} |")
    md.append(f"| Sector Articles | {inv['sectors']} | {'✓ PASS' if inv['sectors'] == EXPECTED_SECTORS else '✗ FAIL'} |")
    ...
    if report["critical"]:
        ...
        # Group by type
        broken_links = [i for i in report["critical"] if "broken_link" in i]
        format_issues = [i for i in report["critical"] if "issue" in i]

        if broken_links:
            md.append(f"### Broken Links ({len(broken_links)} issues)")
            md.append("")
            for issue in broken_links[:10]:
                md.append(f"- **File:** {issue['file']}")
                md.append(f"  **Link:** `{issue['broken_link']}`")
                md.append("")
            if len(broken_links) > 10:
                md.append(f"... and {len(broken_links) - 10} more broken links\n")

        if format_issues:
            md.append(f"### Format/Structure Issues ({len(format_issues)} issues)")
            md.append("")
    ...
    # Warnings
    if report["warnings"]:
        md.append(f"## Warnings ({len(report['warnings'])} issues)")
        md.append("")

        # Group by file
        seo_warnings = [i for i in report["warnings"] if "issues" in i and any("Missing" in issue or "Too" in issue for issue in i.get("issues", []))]
        nav_warnings = [i for i in report["warnings"] if "unexpected_nav" in i]
        other_warnings = [i for i in report["warnings"] if i not in seo_warnings and i not in nav_warnings]

        if seo_warnings:
            md.append(f"### SEO Issues ({len(seo_warnings)} files)")
            md.append("")
            for issue in seo_warnings[:5]:
                md.append(f"**{issue['file']}**")
                for iss in issue.get("issues", [])[:2]:
                    md.append(f"- {iss}")
                md.append("")
            if len(seo_warnings) > 5:
                md.append(f"... and {len(seo_warnings) - 5} more SEO warnings\n")

        if nav_warnings:
            md.append(f"### Navigation Issues ({len(nav_warnings)} files)")
            md.append("")
            for issue in nav_warnings[:5]:
                md.append(f"- **{issue['file']}:** Unexpected nav items: {', '.join(issue.get('unexpected_nav', []))}")
            if len(nav_warnings) > 5:
                md.append(f"\n... and {len(nav_warnings) - 5} more\n")

    md.append("")
    md.append("---")
    md.append("")
    md.append(f"**Report Generated:** 2026-03-04")
    md.append(f"**Total Checks Performed:** 8")
    md.append(f"**Files Analyzed:** {report['inventory']['total_html_files']}")
...
if __name__ == "__main__":
    ...
    print("AUDIT SUMMARY")
    print("="*70)
    print(f"Critical Issues:     {audit_report['summary']['critical_issues']:>6}")
    print(f"Warnings:            {audit_report['summary']['warnings']:>6}")
    print(f"Info Messages:       {audit_report['summary']['info_messages']:>6}")
    print(f"Total Issues:        {audit_report['summary']['total_issues']:>6}")
    print("-"*70)
    print("")
    print("FILE INVENTORY")
    print("-"*70)
    inv = audit_report['inventory']
    print(f"Total HTML Files:    {inv['total_html_files']:>6}")
    print(f"Country Articles:    {inv['countries']:>6}")
    print(f"Company Articles:    {inv['companies']:>6} (expected {EXPECTED_COMPANIES}) {'✓' if inv['companies'] == EXPECTED_COMPANIES else '✗'}")
    print(f"Sector Articles:     {inv['sectors']:>6} (expected {EXPECTED_SECTORS}) {'✓' if inv['sectors'] == EXPECTED_SECTORS else '✗'}")
    print(f"Browse Pages:        {inv['browse_pages']:>6}")
    print(f"Root Pages:          {inv['root_pages']:>6}")
    print(f"Other Files:         {inv['other']:>6}")
    print("")

    if audit_report['critical']:
        print("CRITICAL ISSUES (first 10):")
        print("-"*70)
        for i, issue in enumerate(audit_report['critical'][:10], 1):
            if "broken_link" in issue:
                print(f"{i}. [BROKEN LINK] {issue['file']}")
                print(f"   → {issue['broken_link']}")
            else:
                print(f"{i}. {issue['file']}")
                print(f"   → {issue['issue']}")
        if len(audit_report['critical']) > 10:
            print(f"\n... and {len(audit_report['critical']) - 10} more critical issues")
        print("")
